...
export default function Home() {
  ...
  if (!session) return <main className="center"><form className="login-card" onSubmit={login}><span className="eyebrow">DATEND · FINANCE OPS</span><div className="brand-icon"><Landmark /></div><h1>Conciliação Financeira</h1><p>Gestão financeira simples, segura e orientada por dados.</p><label>E-mail<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required /></label><label>Senha<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required /></label>{notice && <div className="notice error">{notice}</div>}<button className="primary" type="submit">Entrar</button></form></main>;
  ...
  return <div className="shell">
    ...
    <main className="content"><header><div><span className="eyebrow">CENTRAL FINANCEIRA</span><h1>Conciliação Financeira</h1><p>Extratos, saldos e lançamentos contábeis em um único fluxo.</p></div><div className="user-chip">{session.user.email}</div></header>
