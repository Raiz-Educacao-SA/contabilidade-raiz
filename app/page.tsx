...
export default function Home() {
  ...
  return <div className="shell">
    ...
    <main className="content"><header><div><span className="eyebrow">CENTRAL FINANCEIRA</span><h1>{activeModule.title}</h1><p>{activeModule.description}</p></div><div className="user-chip">{session.user.email}</div></header>
      ...
      {selectedModule === "bancaria" && tab === "conciliacao" && <section className="panel"><div className="panel-title"><div><h2>Nova conciliação</h2><p>Envie os arquivos; o sistema identifica a conta automaticamente pelo extrato e pela planilha.</p></div><button className="secondary" disabled={!results.length && !pinned.length} onClick={() => exportReport([...pinned.flatMap((item) => item.rows), ...results], `conciliacao_${competence}`)}><Download />Baixar relatório</button></div><div className="form-grid"><label>Planilha contábil<input type="file" accept=".xlsx,.xlsm" onChange={(e) => readAccounting(e.target.files?.[0])} /></label><label>Extrato bancário<input type="file" accept=".xlsx,.xlsm" onChange={(e) => readBank(e.target.files?.[0])} /></label><label>Conta identificada<input readOnly value={detectedAccount ? `${detectedAccount.code} — ${detectedAccount.name}` : bank.length ? "Aguardando correspondência na planilha contábil" : "Envie o extrato bancário"} /></label><label>Tolerância de dias<input type="number" min="0" max="15" value={toleranceDays} onChange={(e) => setToleranceDays(Number(e.target.value))} /></label><label>Tolerância de valor<input type="number" min="0" step="0.01" value={toleranceValue} onChange={(e) => setToleranceValue(Number(e.target.value))} /></label></div><button className="primary run" onClick={run}><ArrowLeftRight />Executar conciliação</button>{results.length > 0 && <><ResultBlock rows={results} /><button className="secondary pin-result" onClick={pinResult}><Pin />Fixar conciliação e montar próximo banco</button></>}{pinned.length > 0 && <section className="pinned-area"><div className="pinned-title"><div><Pin /><div><h3>Conciliações fixadas</h3><p>Os bancos abaixo permanecem na tela enquanto você monta os próximos.</p></div></div><b>{pinned.length} banco(s)</b></div>{pinned.map((item) => <article className="pinned-card" key={item.id}><header><div><span>EXTRATO FIXADO</span><h3>{item.bankName}</h3><p>Conta bancária {item.bankAccount || "identificada pelo arquivo"} · Contábil {item.accountCode} — {item.accountName}</p></div><button className="unpin" title="Remover da tela" onClick={() => setPinned((items) => items.filter((current) => current.id !== item.id))}><X />Remover</button></header><ResultBlock rows={item.rows} /></article>)}</section>}</section>}
