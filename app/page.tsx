...
export default function Home() {
  ...
  const [bankMetadata, setBankMetadata] = useState<BankMetadata>({ agency: "", account: "", period: "", name: "", openingBalance: null, closingBalance: null });
  ...
  async function readBank(file?: File) { if (!file) return; try { const parsed = parseBank(await file.arrayBuffer()); setBank(parsed.rows); setBankMetadata(parsed.metadata); setBankName(file.name); setResults([]); setNotice("Extrato carregado. A conta será identificada automaticamente."); } catch (error) { setNotice((error as Error).message); } }
  ...
  function pinResult() { if (!results.length || !detectedAccount) return; setPinned((items) => [...items, { id: crypto.randomUUID(), bankName, bankAccount: bankMetadata.account, accountCode: detectedAccount.code, accountName: detectedAccount.name, rows: results }]); setBank([]); setBankName(""); setBankMetadata({ agency: "", account: "", period: "", name: "", openingBalance: null, closingBalance: null }); setResults([]); setNotice("Conciliação fixada. Agora carregue o extrato do próximo banco."); }
  ...
  return <div className={`shell ${selectedModule === "book" ? "book-shell" : ""}`}>
    <aside><div className="logo"><Image className="logo-image" src="/logo-raiz.png" alt="Raiz Educação" width={54} height={54} priority /><div><b>CONTABILIDADE</b><span>Raiz Educação</span></div></div><button className="module-back" onClick={() => { if (selectedArea === "financeiro") setSelectedModule(null); else { setSelectedArea(null); setSelectedModule(null); } }}><ChevronLeft />{selectedArea === "financeiro" ? "Voltar ao Financeiro" : "Voltar aos módulos"}</button><div className="current-module"><ActiveModuleIcon /><span>{activeModule.title}</span></div>{selectedModule === "bancaria" && <nav>{([{ id: "conciliacao", label: "Conciliação", icon: ArrowLeftRight }, { id: "contas", label: "Contas", icon: WalletCards }, { id: "extratos", label: "Extratos", icon: Upload }, { id: "saldos", label: "Saldos", icon: FileSpreadsheet }] as const).map(({ id, label, icon: Icon }) => <button key={id} className={tab === id ? "active" : ""} onClick={() => setTab(id)}><Icon />{label}</button>)}</nav>}<button className="logout" onClick={() => supabase.auth.signOut()}><LogOut />Sair</button></aside>
    <main className={`content ${selectedModule === "book" ? "book-content" : selectedModule === "receita" ? "revenue-content" : ""}`}><header><div><span className="eyebrow">CONTABILIDADE RAIZ</span><h1>{activeModule.title}</h1><p>{activeModule.description}</p></div><div className="user-chip">{session.user.email}</div></header>
      <section className="top-context"><div className="filter-heading"><span className="filter-icon"><SlidersHorizontal /></span><div><b>Filtros da análise</b><small>Selecione a empresa e a competência</small></div></div><div className="filter-fields"><label className="company-control"><span>Empresa</span><select value={companyId} onChange={(e) => setCompanyId(e.target.value)}>{companies.map((item) => <option key={item.empresa_id} value={item.empresa_id}>{item.empresas?.codcoligada} — {item.empresas?.razao_social}</option>)}</select></label><div className="competence-control"><label><span>Ano</span><input type="number" value={year} onChange={(e) => setYear(Number(e.target.value))} /></label><label><span>Mês</span><select value={month} onChange={(e) => setMonth(Number(e.target.value))}>{months.map((name, index) => <option key={name} value={index + 1}>{name}</option>)}</select></label></div></div></section>
      {notice && <div className="notice">{notice}</div>}
      {selectedModule === "bancaria" && tab === "conciliacao" && <MonthlyReconciliationPanel accounts={accounts} competence={competence} companyId={companyId} companyCode={company?.empresas?.codcoligada ?? ""} companyName={`${company?.empresas?.codcoligada ?? ""} — ${company?.empresas?.razao_social ?? ""}`} reconciledBy={session.user.email ?? ""} accessToken={session.access_token} />}
      ...
      {selectedModule === "contabil" && <PisCofinsAssessment companyCode={company?.empresas?.codcoligada ?? ""} companyName={`${company?.empresas?.codcoligada ?? ""} — ${company?.empresas?.razao_social ?? ""}`} competence={competence} accessToken={session.access_token} />}
      {selectedModule !== "bancaria" && selectedModule !== "book" && selectedModule !== "receita" && selectedModule !== "contabil" && <section className="panel module-workspace"><ActiveModuleIcon /><span className="eyebrow">MÓDULO SELECIONADO</span><h2>{activeModule.title}</h2><p>A empresa <b>{company?.empresas?.codcoligada} — {company?.empresas?.razao_social}</b> já está selecionada. Este espaço está preparado para receber as regras, contratos e relatórios deste módulo.</p><div className="coming-next"><Building2 /><div><b>Estrutura pronta para evolução</b><span>Empresa, competência e permissões já compartilham a mesma base da conciliação bancária.</span></div></div></section>}
    </main>
  </div>;
}

function ResultBlock({ rows }: { rows: MatchRow[] }) {
  const totals = rows.reduce<Record<string, number>>((acc, row) => ({ ...acc, [row.status]: (acc[row.status] ?? 0) + 1 }), {});
  const totalDifference = rows.reduce((sum, row) => sum + (row.status === "Somente no banco" ? Math.abs(row.bankValue ?? 0) : row.status === "Somente na contabilidade" ? Math.abs(row.accountingValue ?? 0) : 0), 0);
