...
import { ArrowLeftRight, Download, FileSpreadsheet, Landmark, LogOut, Plus, Save, Upload, WalletCards } from "lucide-react";
import type { Session } from "@supabase/supabase-js";
import { configured, supabase } from "@/lib/supabase";
import { AccountingRow, BankRow, MatchRow, brl, exportReport, parseAccounting, parseBank, reconcile } from "@/lib/reconciliation";
...
export default function Home() {
  ...
  const filteredAccounting = useMemo(() => {
    const code = accounts.find((item) => item.id === selectedAccount)?.conta_contabil.replace(/\D/g, "");
    return code ? accounting.filter((row) => row.account.replace(/\D/g, "").includes(code)) : [];
  }, [accounting, accounts, selectedAccount]);
  ...
  async function readBank(file?: File) { if (!file) return; try { setBank(await parseBank(await file.arrayBuffer())); setBankName(file.name); setNotice("Extrato carregado."); } catch (error) { setNotice((error as Error).message); } }
  function run() { if (!bank.length || !filteredAccounting.length) return setNotice("Envie os arquivos e confirme a conta contábil do extrato."); setResults(reconcile(bank, filteredAccounting, toleranceDays, toleranceValue)); setNotice("Conciliação concluída."); }
  ...
  if (!session) return <main className="center"><form className="login-card" onSubmit={login}><span className="eyebrow">DATEND · FINANCE OPS</span><div className="brand-icon"><Landmark /></div><h1>Conciliação bancária</h1><p>Conferência financeira simples, segura e orientada por dados.</p><label>E-mail<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required /></label><label>Senha<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required /></label>{notice && <div className="notice error">{notice}</div>}<button className="primary" type="submit">Entrar</button></form></main>;
  ...
  return <div className="shell">
    <aside><div className="logo"><Landmark /><div><b>DATEND</b><span>Finance ops</span></div></div><nav>{([{ id: "conciliacao", label: "Conciliação", icon: ArrowLeftRight }, { id: "contas", label: "Contas", icon: WalletCards }, { id: "extratos", label: "Extratos", icon: Upload }, { id: "saldos", label: "Saldos", icon: FileSpreadsheet }] as const).map(({ id, label, icon: Icon }) => <button key={id} className={tab === id ? "active" : ""} onClick={() => setTab(id)}><Icon />{label}</button>)}</nav><div className="sidebar-context"><span>Empresa</span><select value={companyId} onChange={(e) => setCompanyId(e.target.value)}>{companies.map((item) => <option key={item.empresa_id} value={item.empresa_id}>{item.empresas?.codcoligada} — {item.empresas?.razao_social}</option>)}</select><div className="period"><label>Ano<input type="number" value={year} onChange={(e) => setYear(Number(e.target.value))} /></label><label>Mês<select value={month} onChange={(e) => setMonth(Number(e.target.value))}>{months.map((name, index) => <option key={name} value={index + 1}>{name}</option>)}</select></label></div></div><button className="logout" onClick={() => supabase.auth.signOut()}><LogOut />Sair</button></aside>
    <main className="content"><header><div><span className="eyebrow">CENTRAL FINANCEIRA</span><h1>Conciliação bancária</h1><p>Extratos, saldos e lançamentos contábeis em um único fluxo.</p></div><div className="user-chip">{session.user.email}</div></header>
      ...
      {tab === "conciliacao" && <section className="panel"><div className="panel-title"><div><h2>Nova conciliação</h2><p>Envie os arquivos, confirme a conta e execute as regras.</p></div><button className="secondary" disabled={!results.length} onClick={() => exportReport(results, `conciliacao_${competence}`)}><Download />Baixar relatório</button></div><div className="form-grid"><label>Planilha contábil<input type="file" accept=".xlsx,.xlsm" onChange={(e) => readAccounting(e.target.files?.[0])} /></label><label>Extrato bancário<input type="file" accept=".xlsx,.xlsm" onChange={(e) => readBank(e.target.files?.[0])} /></label><label>Conta do extrato<select value={selectedAccount} onChange={(e) => setSelectedAccount(e.target.value)}>{accounts.map((item) => <option key={item.id} value={item.id}>{item.banco} | {item.conta_bancaria} | Contábil {item.conta_contabil}</option>)}</select></label><label>Tolerância de dias<input type="number" min="0" max="15" value={toleranceDays} onChange={(e) => setToleranceDays(Number(e.target.value))} /></label><label>Tolerância de valor<input type="number" min="0" step="0.01" value={toleranceValue} onChange={(e) => setToleranceValue(Number(e.target.value))} /></label></div><button className="primary run" onClick={run}><ArrowLeftRight />Executar conciliação</button>{results.length > 0 && <><div className="result-metrics"><article><b>{counts["Conciliado"] ?? 0}</b><span>Conciliados</span></article><article><b>{counts["Possível conciliação"] ?? 0}</b><span>Possíveis</span></article><article><b>{counts["Somente no banco"] ?? 0}</b><span>Somente no banco</span></article><article><b>{counts["Somente na contabilidade"] ?? 0}</b><span>Somente na contabilidade</span></article><article><b>{brl(difference)}</b><span>Diferença de conciliação</span></article></div><div className="table-wrap"><table><thead><tr><th>Status</th><th>Data banco</th><th>Histórico</th><th>Valor banco</th><th>Data contábil</th><th>Valor contábil</th></tr></thead><tbody>{results.map((row, index) => <tr key={index}><td><span className={`status s${row.status.charAt(0)}`}>{row.status}</span></td><td>{row.bankDate?.toLocaleDateString("pt-BR", { timeZone: "UTC" })}</td><td>{row.description}</td><td>{row.bankValue == null ? "" : brl(row.bankValue)}</td><td>{row.accountingDate?.toLocaleDateString("pt-BR", { timeZone: "UTC" })}</td><td>{row.accountingValue == null ? "" : brl(row.accountingValue)}</td></tr>)}</tbody></table></div></>}</section>}
      ...
      {tab === "saldos" && <BalancePanel companyId={companyId} competence={competence} accounts={accounts} canWrite={canWrite} userId={session.user.id} onNotice={setNotice} />}
    </main>
  </div>;
}
