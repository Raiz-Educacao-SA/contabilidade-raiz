...
export default function Home() {
  ...
  return <div className="shell">
    ...
    <main className="content"><header><div><span className="eyebrow">CONTABILIDADE RAIZ</span><h1>{activeModule.title}</h1><p>{activeModule.description}</p></div><div className="user-chip">{session.user.email}</div></header>
      <section className="top-context"><label className="company-control"><span>Empresa</span><select value={companyId} onChange={(e) => setCompanyId(e.target.value)}>{companies.map((item) => <option key={item.empresa_id} value={item.empresa_id}>{item.empresas?.codcoligada} — {item.empresas?.razao_social}</option>)}</select></label><div className="competence-control"><label><span>Ano</span><input type="number" value={year} onChange={(e) => setYear(Number(e.target.value))} /></label><label><span>Mês</span><select value={month} onChange={(e) => setMonth(Number(e.target.value))}>{months.map((name, index) => <option key={name} value={index + 1}>{name}</option>)}</select></label></div></section>
