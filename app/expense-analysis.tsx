...
export default function ExpenseAnalysis({ companyCode, companyName, competence, accessToken }: Props) {
  ...
  function exportAnalysis() {
    ...
    const movementHeaders = ["IDMOV", "Ticket Zeev", "Data saída", "Fornecedor", "CNPJ/CPF", "Natureza", "Reduzido", "Conta contábil", "Descrição", "Valor", "Coligada", "Filial", "Tipo movimento", "Número movimento", "Data emissão", "Usuário"];
    const movements = movementRecords.map((record) => [
      record.IDMOV, record.TICKET, isoDate(record.DATASAIDA), record.NOMEFANTASIA || record.NOME, record.CGCCFO, "DÉBITO", record.REDUZIDO, record.DEBITO, record.DESCRICAO, numberValue(record.VALOR),
    ...
    movementSheet["!autofilter"] = { ref: `A1:P${Math.max(1, movements.length + 1)}` };
    movementSheet["!freeze"] = { xSplit: 0, ySplit: 1 };
    movementSheet["!cols"] = [{ wch: 12 }, { wch: 16 }, { wch: 13 }, { wch: 40 }, { wch: 20 }, { wch: 12 }, { wch: 12 }, { wch: 20 }, { wch: 36 }, { wch: 16 }, { wch: 10 }, { wch: 9 }, { wch: 16 }, { wch: 18 }, { wch: 13 }, { wch: 20 }];
