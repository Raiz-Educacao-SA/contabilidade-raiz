...
import { AlertTriangle, Building2, CheckCircle2, Download, Eye, EyeOff, RefreshCw, Search } from "lucide-react";
...
type DivergentEntry = AccountingEntry & { companyCode: string; companyName: string; side: "Receber" | "Pagar"; crossing: string };
...
export default function IntercompanyAnalysis({ companies, competence, accessToken }: { companies: CompanyOption[]; competence: string; accessToken: string }) {
  ...
  const compareEntries = (row: AnalysisRow, creditorRows: AccountingEntry[], debtorRows: AccountingEntry[]) => {
    ...
    return [
      ...unmatchedReceivables.map((entry) => ({ ...entry, companyCode: row.creditorCode, companyName: row.creditorName, side: "Receber" as const, crossing })),
      ...payables.filter((_, index) => !matchedPayables.has(index)).map((entry) => ({ ...entry, companyCode: row.debtorCode, companyName: row.debtorName, side: "Pagar" as const, crossing })),
    ];
  };
  ...
  const summary = useMemo(() => ({
    ...
    difference: results.reduce((sum, row) => sum + Math.abs(row.difference), 0),
  }), [results]);
  const filtered = useMemo(() => results.filter((row) => (showReconciled || row.status !== "Conciliado") && (nature === "Todas" || row.nature === nature) && (!search.trim() || `${row.creditorCode} ${row.creditorName} ${row.debtorCode} ${row.debtorName} ${row.receivableAccount} ${row.payableAccount}`.toLowerCase().includes(search.toLowerCase()))), [results, showReconciled, nature, search]);

  function exportResults() {
    if (!results.length) return;
    const rows = results.map((row) => ({ Natureza: row.nature, "Empresa credora": `${row.creditorCode} — ${row.creditorName}`, "Empresa devedora": `${row.debtorCode} — ${row.debtorName}`, "Conta a receber": row.receivableAccount, "Reduzido a receber": row.receivableReduced, "Saldo a receber": row.receivableBalance, "Conta a pagar": row.payableAccount, "Reduzido a pagar": row.payableReduced, "Saldo a pagar": row.payableBalance, Diferença: row.difference, Situação: row.status }));
