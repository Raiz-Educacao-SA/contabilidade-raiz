...
type AnalysisRow = {
  ...
  status:
    | "Conciliado"
    | "Divergente"
    | "Conta a receber ausente"
    | "Conta a pagar ausente"
    | "Contas ausentes";
};
...
const tolerance = 1;
const hasMonthlyMovement = (value: number) =>
...
const mutualPayableByCounterparty: Record<string, string[]> = {
  ...
  "2": ["2.3.1.03.01.07"],
  "3": ["2.3.1.03.01.17"],
  "4": ["2.3.1.03.01.10", "2.3.1.03.02.02"],
  "5": ["2.3.1.03.01.08"],
  "6": ["2.3.1.03.01.09"],
  "8": ["2.3.1.03.01.14"],
  "9": ["2.3.1.03.01.15"],
  ...
  "23": ["2.3.1.03.01.37"],
  "24": ["2.3.1.03.01.38"],
  "25": ["2.1.9.01.03.09", "2.3.1.03.01.29"],
  "26": ["2.3.1.03.01.39"],
  "27": ["2.3.1.03.01.44"],
  "28": ["2.3.1.03.01.45"],
  "29": ["2.3.1.03.01.48"],
...
const holdingRules: Array<{
  ...
  {
    nature: "Almoxarifado",
    receivablePrefix: "1.1.2.03.06",
    payableAccount: "2.1.7.01.02.15",
  },
  {
    nature: "Transação",
    receivablePrefix: "1.1.2.03.05",
    payableAccount: "2.1.7.01.02.16",
  },
];
const mutualReceivableAccounts = new Set(
  Object.values(mutualReceivableByCounterparty),
);
const mutualPayableAccounts = new Set(
  Object.values(mutualPayableByCounterparty).flat(),
);

const intercompanyAccountGroup = (account: string): Nature | null => {
  if (
    mutualReceivableAccounts.has(account) ||
    mutualPayableAccounts.has(account)
  )
    return "Mútuo";
  return (
    holdingRules.find(
      (rule) =>
        rule.payableAccount === account ||
        account.startsWith(`${rule.receivablePrefix}.`),
    )?.nature || null
  );
};
const isIntercompanyAccount = (account: string) =>
  Boolean(intercompanyAccountGroup(account));
const intercompanyAccountNature = (account: string) =>
  mutualReceivableAccounts.has(account) ||
  holdingRules.some((rule) => account.startsWith(`${rule.receivablePrefix}.`))
    ? "Ativo intercompany"
    : "Passivo intercompany";
...
function sumAccounts(rows: BalanceRow[], accounts: string[]) {
  const found = accounts
    .map((account) => exact(rows, account))
...
function crossingDiagnosis(row: AnalysisRow) {
  if (row.status === "Conta a receber ausente")
    return `${row.debtorCode} — ${row.debtorName} possui movimento no passivo, mas ${row.creditorCode} — ${row.creditorName} não possui o movimento no ativo correspondente.`;
  if (row.status === "Conta a pagar ausente")
    return `${row.creditorCode} — ${row.creditorName} possui movimento no ativo, mas ${row.debtorCode} — ${row.debtorName} não possui o movimento no passivo correspondente.`;
  if (row.status === "Contas ausentes")
    return "As contas esperadas não foram localizadas nas duas empresas.";
  if (row.difference > tolerance)
    return `O movimento do ativo de ${row.creditorCode} — ${row.creditorName} supera a contrapartida do passivo de ${row.debtorCode} — ${row.debtorName}.`;
  if (row.difference < -tolerance)
    return `O movimento do passivo de ${row.debtorCode} — ${row.debtorName} supera a contrapartida do ativo de ${row.creditorCode} — ${row.creditorName}.`;
...
export default function IntercompanyAnalysis({
  companies,
  selectedCompanyCode,
  competence,
  accessToken,
  ...
  const [activeGroup, setActiveGroup] = useState<Nature>("Mútuo");
  ...
  const selectedCompany = normalizedCompanies.find(
    (item) => item.code === selectedCode,
  );
  const isUpdatedForSelection =
    updatedCompetence === competence && updatedCompanyCode === selectedCode;

  const intercompanyAccounts = useMemo(() => {
    if (!selectedCompany) return [];
    return (balances[selectedCompany.code] || [])
      .filter(
        (row) =>
          isIntercompanyAccount(row.account) &&
          hasMonthlyMovement(row.movement),
      )
      .map((row) => ({
        ...row,
        companyCode: selectedCompany.code,
        companyName: selectedCompany.name,
        group: intercompanyAccountGroup(row.account) as Nature,
        nature: intercompanyAccountNature(row.account),
      }))
      .sort(
        (a, b) =>
          a.group.localeCompare(b.group) || a.account.localeCompare(b.account),
      );
  }, [balances, selectedCompany]);

  const sourceGroups = useMemo(
    () =>
      intercompanyGroups.map((group) => {
        const rows = intercompanyAccounts.filter((row) => row.group === group);
        return {
          group,
          rows,
          movement: rows.reduce((total, row) => total + row.movement, 0),
        };
      }),
  ...
  async function updateBalances() {
    setLoading(true);
    setUpdatedCompetence("");
    setUpdatedCompanyCode("");
        ...
        batch.map(async (company) => {
          try {
            const response = await fetch(
              `/api/totvs/trial-balance?company=${company.code}&competence=${competence}`,
              {
            ...
            if (!response.ok)
              throw new Error(
                payload.error || "Falha ao consultar o balancete.",
              );
            return { company, rows: (payload.rows || []) as BalanceRow[] };
          } catch (error) {
            return { company, error: (error as Error).message };
          }
        }),
      );
      responses.forEach((item) => {
        if ("rows" in item) loaded[item.company.code] = item.rows || [];
        else failures.push(`${item.company.code} — ${item.company.name}`);
      });
    }
    setBalances(loaded);
    setLoading(false);
    ...
    if (count > 0) {
      setUpdatedCompetence(competence);
      setUpdatedCompanyCode(selectedCode);
    }
    setMessage(
      failures.length
        ? `${count} balancete(s) carregado(s). Não foi possível consultar: ${failures.join(", ")}.`
        : `${count} balancete(s) carregado(s) para ${competence.slice(5)}/${competence.slice(0, 4)}.`,
    );
  }
  ...
  function analyze() {
    ...
    const appendMutual = (creditor: CompanyOption, debtor: CompanyOption) => {
      ...
      if (
        !hasMonthlyMovement(receivableMovement) &&
        !hasMonthlyMovement(payable.value)
      )
        return;
      const difference = receivableMovement + payable.value;
      output.push({
        id: `mutuo-${creditor.code}-${debtor.code}`,
        nature: "Mútuo",
    ...
    if (holding && balances[holdingCode])
      holdingRules.forEach((rule) =>
        available
          .filter(
            (item) =>
              item.code !== holdingCode &&
          ...
          .forEach((counterparty) => {
            ...
            if (
              !hasMonthlyMovement(receivableMovement) &&
              !hasMonthlyMovement(payableMovement)
            )
              return;
            const difference = receivableMovement + payableMovement;
            output.push({
              id: `${rule.nature}-${counterparty.code}`,
              nature: rule.nature,
              creditorCode: holdingCode,
    ...
    setResults(output);
    setActiveGroup(
      intercompanyGroups.find((group) =>
        output.some((row) => row.nature === group),
      ) || "Mútuo",
    );
    setAnalyzing(false);
    setHasAnalyzed(true);
    const treatmentCount = output.filter(
      (row) => row.status !== "Conciliado",
    ).length;
    setMessage(
      treatmentCount
        ? `${treatmentCount} divergência(s) de movimento encontrada(s) para tratamento.`
        : "Nenhuma divergência encontrada. Os movimentos mensais estão conciliados.",
    );
  }
  ...
  async function loadAccountingEntries(company: string, accounts: string[]) {
    ...
    if (!response.ok)
      throw new Error(
        payload.error || "Não foi possível consultar os lançamentos contábeis.",
      );
      ...
      intercompanyGroups.map((group) => {
        ...
        return {
          ...
          rows: rows.filter(
            (row) =>
              !search.trim() ||
              `${row.creditorCode} ${row.creditorName} ${row.debtorCode} ${row.debtorName} ${row.receivableAccount} ${row.payableAccount}`
                .toLowerCase()
  ...
  function exportResults() {
    ...
    const exportRows = (items: AnalysisRow[]) =>
      items.map((row) => ({
        Grupo: row.nature,
        ...
        Situação: row.status,
        Diagnóstico: crossingDiagnosis(row),
      }));
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(
      workbook,
      XLSX.utils.json_to_sheet(exportRows(results)),
      "Conferência mensal",
    );
    intercompanyGroups.forEach((item) =>
      XLSX.utils.book_append_sheet(
        workbook,
        XLSX.utils.json_to_sheet(
          exportRows(results.filter((row) => row.nature === item)),
        ),
        item,
      ),
    );
    ...
    XLSX.writeFile(
      workbook,
      `Intercompany_${selectedCode}_${competence.replace("-", ".")}.xlsx`,
    );
  }

  function selectAnalysisGroup(group: Nature) {
  ...
  return (
    <section className="module-panel intercompany-panel">
      <div className="intercompany-heading">
        <div>
          <h2>Análise Intercompany por movimento mensal</h2>
          <p>
            Mútuo, Almoxarifado e Transação são conferidos separadamente entre
            as empresas na competência selecionada.
          </p>
        </div>
        <div className="intercompany-actions">
          <button
            className="primary"
            onClick={() => void updateBalances()}
            disabled={loading || !selectedCompany}
          >
            <RefreshCw className={loading ? "spin" : ""} />
            {loading ? "Atualizando..." : "Atualizar Intercompany"}
          </button>
          <button
            onClick={analyze}
            disabled={!isUpdatedForSelection || analyzing}
          >
            {analyzing ? "Analisando..." : "Analisar movimentos"}
          </button>
      ...
      {(loading || analyzing) && (
        <div className="intercompany-processing">
          <RefreshCw className="spin" />
          <b>
            {loading
              ? "Consultando os balancetes..."
              : "Comparando os movimentos mensais..."}
          </b>
          <span>Aguarde a conclusão da conferência entre as empresas.</span>
        </div>
      )}
      {isUpdatedForSelection && !hasAnalyzed && (
        <div className="intercompany-source-view">
          <div className="intercompany-source-heading">
            <b>Movimentos Intercompany da empresa selecionada</b>
            <span>
              {selectedCompany?.code} — {selectedCompany?.name} ·{" "}
              {competence.slice(5)}/{competence.slice(0, 4)}
            </span>
          </div>
          <div className="intercompany-source-groups">
            {sourceGroups.map((section) => (
              <section
                className="intercompany-source-group"
                key={section.group}
              >
                <header>
                  <div>
                    <b>{section.group}</b>
                    <span>{section.rows.length} conta(s)</span>
                  </div>
                  <strong>{money.format(section.movement)}</strong>
                </header>
                <div className="table-wrap intercompany-source-table">
                  <table>
                    <thead>
                      <tr>
                        <th>Natureza</th>
                        ...
                        <th>Reduzido</th>
                        <th>Descrição</th>
                        <th>Movimento do mês</th>
                      </tr>
                    </thead>
                    <tbody>
                      {section.rows.length ? (
                        section.rows.map((row) => (
                          <tr key={`${section.group}-${row.account}`}>
                            <td>{row.nature}</td>
                            <td>
                              <b>{row.account}</b>
                            </td>
                            ...
                            <td className={row.movement < 0 ? "negative" : ""}>
                              <b>{money.format(row.movement)}</b>
                            </td>
                          </tr>
                        ))
                      ) : (
                        <tr>
                          <td colSpan={5} className="empty-row">
                            Nenhuma conta com movimento localizada neste grupo.
                          </td>
                        </tr>
                      )}
                    </tbody>
      ...
      {hasAnalyzed && (
        <>
          <div className="intercompany-summary intercompany-summary-divergences">
            <article>
              <span>Cruzamentos analisados</span>
              <b>{summary.crossings}</b>
            </article>
            <article>
              <span>Movimentos conciliados</span>
              <b>{summary.reconciled}</b>
            </article>
            <article className={summary.treatment ? "has-warning" : ""}>
              <span>Divergências de movimento</span>
              <b>{summary.treatment}</b>
            </article>
            <article
              className={summary.difference > tolerance ? "has-warning" : ""}
            >
              <span>Diferença absoluta do movimento</span>
              <b>{money.format(summary.difference)}</b>
            </article>
          </div>
          <div className="intercompany-toolbar">
            <label>
              <Search />
              <input
                value={search}
                onChange={(event) => setSearch(event.target.value)}
                placeholder={`Buscar empresa ou conta em ${activeGroup}`}
              />
            </label>
            <span>
              {activeGroupResult?.rows.length || 0} cruzamento(s) nesta aba
            </span>
          </div>
          <div
            className="intercompany-analysis-tabs"
            role="tablist"
            aria-label="Grupos da análise Intercompany"
          >
            {groupedResults.map((section) => (
              <button
                type="button"
                role="tab"
                aria-selected={activeGroup === section.group}
                className={activeGroup === section.group ? "active" : ""}
                key={section.group}
                onClick={() => selectAnalysisGroup(section.group)}
              >
                <span>{section.group}</span>
                <small>{section.crossings} cruzamento(s)</small>
                <b>{section.treatment} para tratar</b>
              </button>
            ))}
          </div>
          {activeGroupResult && (
            <section
              className="intercompany-group-section intercompany-tab-panel"
              role="tabpanel"
              aria-label={`Análise de ${activeGroupResult.group}`}
            >
              <header className="intercompany-group-heading">
                <div>
                  <b>{activeGroupResult.group}</b>
                  <span>
                    Conferência do movimento da competência entre as empresas
                  </span>
                </div>
                <div>
                  <small>{activeGroupResult.reconciled} conciliado(s)</small>
                  <strong>{activeGroupResult.treatment} para tratar</strong>
                  <span>
                    Diferença: {money.format(activeGroupResult.difference)}
                  </span>
                </div>
              </header>
              <div className="table-wrap intercompany-group-table">
                <table>
                  <thead>
                    <tr>
                      <th>Outra empresa</th>
                      <th>Empresa com ativo a receber</th>
                      <th>Movimento do ativo</th>
                      <th>Empresa com passivo a pagar</th>
                      <th>Movimento do passivo</th>
                  ...
                  <tbody>
                    {activeGroupResult.rows.length ? (
                      activeGroupResult.rows.map((row) => {
                        const counterpart =
                          row.creditorCode === selectedCode
                            ? { code: row.debtorCode, name: row.debtorName }
                            : {
                                code: row.creditorCode,
                                name: row.creditorName,
                              };
                        return (
                          ...
                          >
                            <td>
                              <b>{counterpart.code}</b> — {counterpart.name}
                            </td>
                            <td>
                            ...
                            <td
                              className={
                                row.receivableMovement < 0 ? "negative" : ""
                              }
                            >
                              <b>{money.format(row.receivableMovement)}</b>
                            </td>
                            <td>
                              <div className="intercompany-account-cell">
                                <strong>
                            ...
                            <td
                              className={
                                row.payableMovement < 0 ? "negative" : ""
                              }
                            >
                              <b>{money.format(row.payableMovement)}</b>
                            </td>
                            <td
                              className={
                            ...
                            >
                              <b>{money.format(row.difference)}</b>
                            </td>
                            <td>
                              <span
                                className={`intercompany-status ${row.status === "Conciliado" ? "ok" : "warning"}`}
                              >
                            ...
                            <td className="intercompany-diagnosis">
                              {crossingDiagnosis(row)}
                            </td>
                            <td>
                              <button
                                className="intercompany-detail-button"
                                onClick={() =>
                                  void identifyDivergentEntries(row)
                                }
                                disabled={loadingEntries}
                              >
                                {loadingEntries && selectedRow?.id === row.id
                                  ? "Buscando..."
                                  : "Identificar"}
                              </button>
                            </td>
                      ...
                      <tr>
                        <td colSpan={9} className="empty-row">
                          Nenhum cruzamento localizado nesta aba para a busca
                          informada.
                        </td>
                      </tr>
                    )}
          ...
          {selectedRow && (
            <div className="intercompany-entry-panel">
              <div className="intercompany-entry-heading">
                <div>
                  <b>Lançamentos do Razão das contas por empresa</b>
                  <span>
                    {selectedRow.creditorCode} × {selectedRow.debtorCode} ·{" "}
                    {selectedRow.nature} · diferença mensal{" "}
                    {money.format(selectedRow.difference)}
                  </span>
                </div>
                <button
                  onClick={() => {
                    ...
                    >
                      <header>
                        <div>
                          ...
                          <span>
                            {group.side} · conta(s) {group.accounts}
                          </span>
                        </div>
                        <strong>
                          {money.format(
                      ...
                      {group.rows.length ? (
                        <div className="table-wrap intercompany-entry-table">
                          <table>
                            <thead>
                              <tr>
                                <th>Conferência</th>
                            ...
                            <tbody>
                              {group.rows.map((entry) => (
                                <tr key={`${group.code}-${entry.id}`}>
                                  <td>
                                    <span
                                      className={`intercompany-status ${entry.matchStatus === "Com contrapartida" ? "ok" : "warning"}`}
                                    >
                                      {entry.matchStatus}
                                    </span>
                                  </td>
                        ...
                        <div className="intercompany-entry-empty">
                          Nenhum lançamento localizado no Razão para a conta e
                          competência informadas.
                        </div>
                      )}
                    </section>
                  ))}
      ...
      {!isUpdatedForSelection && !loading && (
        <div className="intercompany-empty">
          <Building2 />
          <b>Atualize Intercompany para iniciar</b>
          <span>
            Serão conferidos os movimentos da competência entre a empresa
            selecionada e as demais empresas.
          </span>
        </div>
      )}
    </section>
