...
export default function CscAllocation({ companies, competence, accessToken }: { companies: Company[]; competence: string; accessToken: string }) {
  ...
  const calculationList = useMemo(() => list.map((company) => {
    ...
    const rule = company.code === "1" ? "Origem dos custos do CSC" : company.code === "25" ? "Contrato Sarah" : company.code === "18" ? "Contrato Apogeu / Espaço Mágico" : company.code === "20" ? "Contrato Integra" : "Proporcional ao movimento";
    return { ...company, ...movement, rule: calculated?.rule || rule, status: calculated ? "Calculada" : Object.prototype.hasOwnProperty.call(resultMovements, company.code) ? "Base carregada" : "Aguardando base" };
  ...
  return <section className="panel csc-panel">
    ...
    {rows.length && <><div className="csc-company-list-title"><div><b>Rateio dos custos da Raiz entre as coligadas</b><span>Distribuição calculada conforme o movimento mensal</span></div></div><div className="table-wrap csc-table"><table><thead><tr><th>Coligada</th><th>Empresa</th><th>Movimento base</th><th>Participação</th><th>Regra</th><th>Rateio</th><th>Ajuste</th><th>Final</th></tr></thead><tbody>{rows.map((row) => <tr key={row.code}><td><b>{row.code}</b></td><td>{row.name}</td><td>{money.format(row.revenue)}</td><td>{row.share ? percent.format(row.share) : "—"}</td><td>{row.rule}</td><td><b>{money.format(row.calculated)}</b></td><td><input type="number" step=".01" value={adjustments[row.code] || 0} onChange={(e) => setAdjustments({ ...adjustments, [row.code]: Number(e.target.value) })} /></td><td><b>{money.format(row.finalValue)}</b></td></tr>)}</tbody></table></div></>}</>}
  </section>;
}
