...
export default function CscAllocation({ companies, competence, accessToken }: { companies: Company[]; competence: string; accessToken: string }) {
  ...
  async function updateCosts() {
    ...
    try { const value = costValue(await balance("1")); setCostPool(value); setMessage(`${money.format(value)} de custos mensais da Raiz carregados para ${competence.slice(5)}/${competence.slice(0, 4)}. Créditos e estornos foram deduzidos.`); }
  ...
  const calculationList = useMemo(() => list.filter((company) => company.code !== "1").map((company) => {
    const calculated = rows.find((row) => row.code === company.code);
    const rule = company.code === "25" ? "Contrato Sarah" : company.code === "18" ? "Contrato Apogeu / Espaço Mágico" : company.code === "20" ? "Contrato Integra" : "Proporcional ao faturamento";
    return { ...company, revenue: revenues[company.code] || 0, rule: calculated?.rule || rule, status: calculated ? "Calculada" : Object.prototype.hasOwnProperty.call(revenues, company.code) ? "Base carregada" : "Aguardando base" };
  }), [list, revenues, rows]);
  function exportFile() {
  ...
  return <section className="panel csc-panel">
    ...
    <div className="csc-company-list-title"><div><b>Empresas incluídas no cálculo mensal</b><span>{calculationList.length} empresa(s) na competência {competence.slice(5)}/{competence.slice(0, 4)}</span></div></div>
    {rows.length ? <div className="table-wrap csc-table"><table><thead><tr><th>Coligada</th><th>Empresa</th><th>Faturamento</th><th>Participação</th><th>Regra</th><th>Rateio</th><th>Ajuste</th><th>Final</th></tr></thead><tbody>{rows.map((row) => <tr key={row.code}><td><b>{row.code}</b></td><td>{row.name}</td><td>{money.format(row.revenue)}</td><td>{row.share ? percent.format(row.share) : "—"}</td><td>{row.rule}</td><td><b>{money.format(row.calculated)}</b></td><td><input type="number" step=".01" value={adjustments[row.code] || 0} onChange={(e) => setAdjustments({ ...adjustments, [row.code]: Number(e.target.value) })} /></td><td><b>{money.format(row.finalValue)}</b></td></tr>)}</tbody></table></div> : <div className="table-wrap csc-table csc-company-preview"><table><thead><tr><th>Coligada</th><th>Empresa</th><th>Faturamento</th><th>Regra</th><th>Situação</th></tr></thead><tbody>{calculationList.map((row) => <tr key={`preview-${row.code}`}><td><b>{row.code}</b></td><td>{row.name}</td><td>{money.format(row.revenue)}</td><td>{row.rule}</td><td><span className={`csc-company-status ${row.status === "Calculada" ? "done" : row.status === "Base carregada" ? "ready" : ""}`}>{row.status}</span></td></tr>)}</tbody></table></div>}</>}
  </section>;
}
