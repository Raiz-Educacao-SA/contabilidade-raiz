...
async function branchRevenue(baseUrl: string, user: string, password: string, company: string, firstDay: string, lastDay: string) {
  ...
  return Object.entries(totals).map(([branch, movement]) => ({ branch, movement: Math.abs(movement), revenue: Math.abs(movement) })).sort((a, b) => Number(a.branch) - Number(b.branch));
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const branches = request.nextUrl.searchParams.get("byBranch") === "1" ? await branchRevenue(baseUrl, user, password, company, firstDay, lastDay) : [];
