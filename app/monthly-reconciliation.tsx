...
function ReconciliationFormView({ result, companyName, reconciledBy }: { result: AccountResult; companyName: string; reconciledBy: string }) {
  ...
  return <section className="reconciliation-form"><header><span>VISÃO DA CONCILIAÇÃO</span><h2>Ficha de Conciliação Bancária</h2><b className={result.validation.reconciled ? "form-ok" : "form-review"}>{result.validation.reconciled ? "CONCILIADO" : "VERIFICAR"}</b></header><div className="form-identification"><div><span>Empresa</span><b>{companyName}</b></div><div><span>Competência</span><b>{result.competence.split("-").reverse().join("/")}</b></div><div><span>Conciliado por</span><b>{reconciledBy}</b></div><div><span>Nº da conta contábil</span><b>{result.account.code}</b></div><div className="wide"><span>Nome da conta contábil</span><b>{result.account.name}</b></div></div><div className="form-summary"><article><span>Saldo conforme extrato bancário</span><b>{statementBalance == null ? "Não informado" : brl(statementBalance)}</b></article><article><span>Total das pendências detalhadas</span><b>{brl(pendingTotal)}</b></article><article><span>Saldo contábil apurado</span><b>{accountingBalance == null ? "Movimento mensal" : brl(accountingBalance)}</b></article><article><span>Diferença da conciliação</span><b>{brl(result.validation.movementDifference)}</b></article></div><div className="form-sections">{sections.map((section) => <article key={section.key}><div className="form-section-title"><b>{section.key}) {section.title}</b><strong>{brl(total(section.rows))}</strong></div>{section.rows.length === 0 ? <p>Nenhuma pendência nesta categoria.</p> : <div className="table-wrap"><table><thead><tr><th>Data</th><th>Documento/Histórico</th><th>Valor</th><th>Referência</th></tr></thead><tbody>{section.rows.map((row, index) => <tr key={index}><td>{(row.bankDate ?? row.accountingDate)?.toLocaleDateString("pt-BR", { timeZone: "UTC" })}</td><td>{row.description || row.nature || "Lançamento contábil"}</td><td>{brl(Math.abs(row.bankValue ?? row.accountingValue ?? 0))}</td><td>{row.status}</td></tr>)}</tbody></table></div>}</article>)}</div></section>;
}
