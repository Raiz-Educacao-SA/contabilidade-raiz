...
import { ArrowLeftRight, CheckCircle2, Download, FileSpreadsheet, Landmark, Upload, XCircle } from "lucide-react";
...
type AccountResult = Statement & { rows: MatchRow[]; validation: ReturnType<typeof validateMonthly> };

export default function MonthlyReconciliationPanel({ accounts, competence }: { accounts: RegisteredAccount[]; competence: string }) {
  ...
  async function loadAccounting(file?: File) {
    ...
    try {
      ...
      setAccounting(rows); setBankAccounts(discovered); setStatements([]); setResults([]); setFileKey((value) => value + 1);
  ...
  async function loadStatement(file?: File) {
    ...
    try {
      ...
      setStatements(updated); setFileKey((value) => value + 1); setResults([]);
  ...
  function runAll() {
    ...
    const completed = statements.map((statement) => {
      ...
      return { ...statement, rows, validation: validateMonthly(statement.bank, statement.account.rows, statement.metadata) };
    });
  ...
  return <section className="panel monthly-flow">
    <div className="panel-title"><div><h2>Conciliação mensal por movimento</h2><p>A planilha contábil identifica as contas e cria a fila de extratos. A execução ocorre de uma só vez.</p></div><button className="secondary" disabled={!allRows.length} onClick={() => exportReport(allRows, `conciliacao_mensal_${competence}`)}><Download />Relatório consolidado</button></div>
    ...
    {bankAccounts.length > 0 && <><div className="queue-head"><div><h3>2. Extratos solicitados</h3><p>Anexe um extrato por vez. O sistema confirma automaticamente a conta correspondente.</p></div><b>{statements.length}/{bankAccounts.length} recebidos</b></div><div className="account-queue">{bankAccounts.map((account, index) => { const statement = statements.find((item) => item.account.code === account.code); const isNext = next?.code === account.code; return <article key={account.code} className={statement ? "received" : isNext ? "requested" : "waiting"}><span>{statement ? <CheckCircle2 /> : <Landmark />}</span><div><b>{account.code} — {account.name}</b><small>{statement ? `Extrato recebido: ${statement.fileName}` : isNext ? "Aguardando este extrato" : `Aguardando a conta anterior (${index + 1}ª da fila)`}</small></div></article>; })}</div></>}
    ...
    {results.length > 0 && <div className="monthly-results">{results.map((result) => <MonthlyAccountResult key={result.account.code} result={result} />)}</div>}
  </section>;
}
